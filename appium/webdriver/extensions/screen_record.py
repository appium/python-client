...
class ScreenRecord(webdriver.Remote):
    ...
    def start_recording_screen(self: T, **options: Any) -> Union[bytes, str]:
        ...
        Keyword Args:
            ...
            fileFieldName (str): The name of the form field containing the binary payload.
                "file" by default. (Since Appium 1.18.0)
            formFields (dict): form fields mapping. If any entry has the same key as
                `fileFieldName` then it is going to be ignored. (Since Appium 1.18.0)
            headers (dict): Headers mapping (Since Appium 1.18.0)
