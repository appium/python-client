...
BUILT_APPIUM_DIR_PATH = os.path.join(
    os.path.dirname("__file__"), "build", "lib", "appium"
)
...
def get_current_version():
    current = (
        io.open(
            os.path.join(os.path.dirname("__file__"), "appium", "version.py"),
            encoding="utf-8",
        )
        .read()
        .rstrip()
...
def tag_and_generate_changelog(new_version_num):
    ...
    call_bash_script(
        f'git commit {CHANGELOG_PATH} -m "Update changelog for {new_version_num}"'
    )
...
def build() -> None:
    ...
    status, output = subprocess.getstatusoutput(
        f"{os.getenv('PYTHON_BIN_PATH')} setup.py install"
    )
    if status != 0:
...
def assert_files_count_in_package() -> None:
    ...
    if len(original_files) != len(built_files):
        print(
            f"The count of files in '{APPIUM_DIR_PATH}' and '{BUILT_APPIUM_DIR_PATH}' were different."
        )
        ...
        if diff:
            print(
                f"'{APPIUM_DIR_PATH}' has '{diff}' files than {BUILT_APPIUM_DIR_PATH}"
            )
