...
class TestFindByElementWebelement(BaseTestCase):
    def test_find_element_by_path(self) -> None:
        el = self.driver.find_element(
            by=AppiumBy.IOS_PREDICATE, value='wdName == "UICatalog"'
        )
        assert "UICatalog" == el.get_attribute("name")

        c_el = el.find_elements(
            by=AppiumBy.IOS_PREDICATE, value='label == "Action Sheets"'
        )  # type: list
        assert "Action Sheets" == c_el[0].get_attribute("name")

        c_el = el.find_elements(
            by=AppiumBy.IOS_CLASS_CHAIN, value="**/XCUIElementTypeStaticText"
        )
