...
class TestWebDriverWebDriver:
    ...
    def test_create_session(self):
        ...
        assert (
            f"appium/python {appium_version.version} (selenium"
            in request.headers["user-agent"]
        )

        request_json = json.loads(
            httpretty.HTTPretty.latest_requests[0].body.decode("utf-8")
        )
    ...
    def test_create_session_change_session_id(self):
        ...
        options = (
            ...
            .set_capability("app", "path/to/app")
        )
    ...
    def test_create_session_register_uridirect(self):
        ...
        assert (
            "http://localhost2:4800/special/path/wd/hub" == driver.command_executor._url
        )
    ...
    def test_get_events_catches_missing_events(self):
        ...
        assert events == {}
        httpretty.register_uri(
            httpretty.GET, appium_command("/session/1234567890"), body=json.dumps({})
        )
        events = driver.events
        assert events == {}
    ...
    def test_session_catches_error(self, mock_warning):
        ...
        driver = ios_w3c_driver()
        httpretty.register_uri(
            httpretty.GET, appium_command("/session/1234567890"), body=exceptionCallback
        )
        events = driver.events
        assert events == {}
...
class TestSubModuleWebDriver:
    def android_w3c_driver(self, driver_class):
        ...
        httpretty.register_uri(
            httpretty.POST, appium_command("/session"), body=response_body_json
        )
